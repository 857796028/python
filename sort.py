...
import logging
    #https://github.com/Blackice-Yang/tidy-file/edit/master/example.cfg
...
class TidyFile(object):
    ...
    def tidy_sec(self, sec):
        ...
        for item in all_files(root, exp, single_level=single_level):
            ...
            #可以删除
                print("%s move to %s " % (item, target + os.sep + new_name))
                target_files.append(new_name)
            shutil.copy2(item, target)
            app_log.info("%s move to %s " % (item, target))
            #可以删除
            print("正在复制==》"+"%s move to %s " % (item, target))
            os.remove(item)
            #下行可删除
            print("正在删除==》"+item)
            # 英文中带中文的路径有bug
